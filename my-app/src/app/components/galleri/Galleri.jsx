...
export default function Galleri() {
  ...
  return (
    <section ref={sectionRef} className="bg-black py-10 w-full">
      <h1 className="text-white text-2xl text-center font-bold mb-12 mx-auto w-fit">NIGHT CLUB GALLERY</h1>
      ...
      <div className="collage-wrapper mx-auto">
        ...
        <div className="flex flex-col md:flex-row">
          {bottomRow.map((img, i) => {
            ...
            return <GalleriTile key={img.id} src={img.asset.url} alt={img.description} className="w-full md:w-1/3 h-[70vw] md:h-[18vw]" onClick={() => openPopup(globalIndex)} />;
          })}
        </div>
      </div>

      {popupOpen && <GalleriPopup images={images} activeIndex={activeIndex} onClose={closePopup} onPrev={showPrev} onNext={showNext} />}
    </section>
  );
}
